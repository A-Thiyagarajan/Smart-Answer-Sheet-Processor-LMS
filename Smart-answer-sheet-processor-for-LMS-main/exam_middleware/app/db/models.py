...
from sqlalchemy import (
    ...
    Boolean, Enum, ForeignKey, Index, UniqueConstraint, JSON, Uuid
)
...
UUID_VARIANT = Uuid(as_uuid=True).with_variant(UUID(as_uuid=True), "postgresql")
...
class ExaminationArtifact(Base):
    ...
    artifact_uuid = Column(UUID_VARIANT, default=uuid.uuid4, unique=True, nullable=False)
    ...
    transaction_log = Column(JSON_VARIANT, nullable=True, default=list)
...
class AuditLog(Base):
    ...
    # Additional Data
    request_data = Column(JSON_VARIANT, nullable=True)
    response_data = Column(JSON_VARIANT, nullable=True)
    error_details = Column(JSON_VARIANT, nullable=True)
