...
async def _get_dev_staff_user(db: AsyncSession, requested_username: str) -> StaffUser | None:
    """Return a usable staff account for local mock-LMS development mode."""
    ...
    # The password check is performed by the caller; here we only choose the backing user.
    ...
    for staff in staff_users:
        if staff.username == requested_username:
            return staff
...
async def staff_login(
    ...
    if not staff or not verify_password(form_data.password, staff.hashed_password):
        if settings.mock_lms_enabled and (form_data.username, form_data.password) in {
            ("admin", "admin123"),
            ("admin123", "admin123"),
        }:
            staff = await _get_dev_staff_user(db, form_data.username)
        if not staff:
            raise HTTPException(
...
async def student_login(
    ...
    try:
        if settings.mock_lms_enabled:
            user = mock_lms_service.authenticate(credentials.username, credentials.password)
            if not user:
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="Authentication failed: invalid LMS username or password"
                )
            if user.get("role") != "student":
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Faculty accounts cannot sign in through the student portal"
                )
            if credentials.register_number != user["register_number"]:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail=f"Register number must be {user['register_number']} for this LMS account"
                )

            moodle_token = mock_lms_service.build_token(user)
            site_info = mock_lms_service.get_site_info(moodle_token)
...
async def get_current_student_session(
    ...
    if not expires_at:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Session expiry is missing"
        )
