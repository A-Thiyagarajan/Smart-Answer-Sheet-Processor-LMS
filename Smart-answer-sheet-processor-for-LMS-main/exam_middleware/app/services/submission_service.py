...
from app.services.artifact_service import ArtifactService, SubjectMappingService, AuditService, split_subject_session_key
...
class SubmissionService:
    ...
    async def _execute_submission(
        ...
        if settings.mock_lms_enabled:
            user = mock_lms_service.get_user_from_token(moodle_token)
            if not user:
                raise MoodleAPIError("Mock LMS token is invalid or expired")
            submission = mock_lms_service.submit_artifact(
                assignment_id=assignment_id,
                artifact_uuid=str(artifact.artifact_uuid),
                filename=artifact.original_filename,
                subject_code=split_subject_session_key(artifact.parsed_subject_code)[0] or "",
                exam_session=split_subject_session_key(artifact.parsed_subject_code)[1],
                student_username=user["username"],
                register_number=user["register_number"],
            )
            return {
                "assignment_id": assignment_id,
                "steps_completed": ["upload", "save", "finalize"],
                "success": True,
                "submission_id": submission["submission_id"],
                "transaction_id": f"MOCK-{submission['submission_id']}",
                "verified_files": [artifact.original_filename],
        ...
        try:
            ...
            result["success"] = True
            result["transaction_id"] = f"TXN_{artifact.artifact_uuid}_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
